...
void HealthStylePersistance::Persistance::PersistXML(String^ fileName, Object^ persistObject)
{
    ...
    try {
        ...
        if (persistObject->GetType() == List<Nurses^>::typeid) {
            XmlSerializer^ serializadorXML = gcnew XmlSerializer(List<Nurses^>::typeid);
            serializadorXML->Serialize(output, persistObject);
        }
    }
    catch (Exception^ ex) {
...
Object^ HealthStylePersistance::Persistance::LoadXMLData(String^ fileName){
    ...
    try {
        ...
        if (fileName->Equals("Nurses.xml")) {
            res = gcnew List<Nurses^>();
            if (File::Exists(fileName)) {
                serializadorXML = gcnew XmlSerializer(List<Nurses^>::typeid);
                res = (List<Nurses^>^)serializadorXML->Deserialize(sr);
            }
        }
    }
    catch (Exception^ ex) {
...
void HealthStylePersistance::Persistance::NursePersistXLM(String^ fileName, Object^ persistObject){
    FileStream^ archivo;
    StreamWriter^ escritor;
    try {
        archivo = gcnew FileStream(fileName, FileMode::Create, FileAccess::Write);
        escritor = gcnew StreamWriter(archivo);
        if (persistObject->GetType() == List<Nurses^>::typeid) {
            for (int i = 0; i < ((List<Nurses^>^)persistObject)->Count; i++) {
                escritor->WriteLine(((List<Nurses^>^)persistObject)[i]->Id + "," +
                    ((List<Nurses^>^)persistObject)[i]->Name + "," +
                    ((List<Nurses^>^)persistObject)[i]->LastName + "," +
                    ((List<Nurses^>^)persistObject)[i]->Username + "," +
                    ((List<Nurses^>^)persistObject)[i]->Password + "," +
                    ((List<Nurses^>^)persistObject)[i]->DocNumber + "," +
                    ((List<Nurses^>^)persistObject)[i]->Address + "," +
                    ((List<Nurses^>^)persistObject)[i]->Email + "," +
                    ((List<Nurses^>^)persistObject)[i]->PhoneNumber + "," +
                    ((List<Nurses^>^)persistObject)[i]->Birthday + "," +
                    ((List<Nurses^>^)persistObject)[i]->Gender + "," +
                    ((List<Nurses^>^)persistObject)[i]->Speciality + "," +
                    ((List<Nurses^>^)persistObject)[i]->YearsOfWork + "," +
                    ((List<Nurses^>^)persistObject)[i]->PatiensAttended + "," +
                    ((List<Nurses^>^)persistObject)[i]->Schedule + "," +
                    ((List<Nurses^>^)persistObject)[i]->Status);
            }
        }
    }
    catch (Exception^ ex) {
